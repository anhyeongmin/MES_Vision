...
from PySide6.QtWidgets import QHBoxLayout,QLabel,QInputDialog,QFileDialog
...
class RecoveryControls:
    def make_recovery_controls(self,outer):
        row=QHBoxLayout(); self.fault_label=ui_text(QLabel); self.fault_label.setWordWrap(True); self.fault_label.setMaximumHeight(80); row.addWidget(self.fault_label,1)
        ...
        row.addWidget(button(tr('진단 내보내기'),lambda:self.guarded(self.export_diagnostics))); outer.addLayout(row); self.refresh_faults()
