...
class DesktopWindow(ResponsiveWindow,ReadinessControls,RecoveryControls,HistoryControls,QMainWindow):
    def __init__(self,root,runtime,*,start_worker=True):
        ...
        central=QWidget(); central.setObjectName("central"); self.setCentralWidget(central); outer=QVBoxLayout(central)
        header=QHBoxLayout(); self.compact_logo=QLabel(); self.compact_logo.setObjectName("compactBrandLogo"); self.compact_logo.setFixedSize(48,54); header.addWidget(self.compact_logo); title=ui_text(QLabel, tr('MES Vision  /  검사 운영')); title.setObjectName("title"); title.setSizePolicy(QSizePolicy.Preferred,QSizePolicy.Preferred); self.title_label=title; header.addWidget(title)
        self.active_label=ui_text(QLabel, tr('선택된 품목 없음')); self.active_label.setWordWrap(True); self.active_label.setMinimumWidth(0); self.active_label.setSizePolicy(QSizePolicy.Ignored,QSizePolicy.Preferred); header.addWidget(self.active_label,1)
        self.vlm=button("VLM OFF",self.toggle_vlm); self.vlm.setCheckable(True); header.addWidget(self.vlm)
        ...
        outer.addLayout(header); self.status=ui_text(QLabel, tr('장비 · 보정에서 카메라를 연결하세요.')); self.status.setWordWrap(True); outer.addWidget(self.status)
        self.make_recovery_controls(outer)
        body=QHBoxLayout(); self.nav=QListWidget(); ui_text(self.nav.addItems, [tr('1  실시간 검사'),tr('2  품목 설정'),tr('3  장비 · 보정'),tr('4  검사 이력'),tr('5  VLM 추가 분석')])
        self.nav.setObjectName('mainNavigation'); self.nav.setFixedWidth(185)
        self.sidebar=QWidget(); self.sidebar.setObjectName('brandSidebar'); sidebar_layout=QVBoxLayout(self.sidebar)
        self.brand_logo=QLabel(); self.brand_logo.setObjectName('brandLogo'); self.brand_logo.setAlignment(Qt.AlignCenter); self.brand_logo.setFixedHeight(150)
        sidebar_layout.addWidget(self.brand_logo); sidebar_layout.addWidget(self.nav,1); body.addWidget(self.sidebar); self.pages=QStackedWidget(); body.addWidget(self.pages,1); outer.addLayout(body,1)
        ...
        self.init_compact_navigation(header)
        ...
        for label in self.findChildren(QLabel):
            if label is not self.title_label: label.setWordWrap(True)
        self.apply_preferences()
        if self.preferences.value["remember_window"] and self.preferences.value["window_geometry"]:
            self.restoreGeometry(QByteArray.fromHex(self.preferences.value["window_geometry"].encode("ascii")))
    ...
    def apply_preferences(self):
        ...
        self.brand_logo.setPixmap(pixmap.scaled(136,150,Qt.KeepAspectRatio,Qt.SmoothTransformation))
        self.compact_logo.setPixmap(pixmap.scaled(48,54,Qt.KeepAspectRatio,Qt.SmoothTransformation))
        font=QFont(QApplication.font()); font.setPointSizeF(10*scale); QApplication.setFont(font)
        self.nav.ensurePolished()
        nav_text_width=max(self.nav.fontMetrics().horizontalAdvance(self.nav.item(i).text()) for i in range(self.nav.count()))
        self.nav.setFixedWidth(max(round(185*scale),nav_text_width+44))
        self.update_responsive_layout()
    ...
    def make_live(self):
        out=self.page(); row=FlowLayout()
        self.start=button(tr('검사 시작'),lambda:self.guarded(self.begin)); self.pause=button(tr('검사 중단'),self.pause_inspection)
        self.unload=button(tr('모델 해제'),self.unload_engine); row.addWidget(self.start); row.addWidget(self.pause); row.addWidget(self.unload)
        self.auto=ui_text(QCheckBox, tr('자동 분류')); self.auto.toggled.connect(self.auto_changed); row.addWidget(self.auto)
        row.addWidget(button(tr('로봇 정지'),self.stop_robot)); out.addLayout(row)
        out.addWidget(button(tr('현재 영상 수집'),lambda:self.guarded(self.capture_training)))
        ...
        panes=QSplitter(); out.addWidget(panes,1)
        from .image_view import ImagePanel
        self.live_image=ImagePanel(); self.canvas=self.live_image.canvas; self.canvas.selected.connect(self.select_track); panes.addWidget(self.live_image)
        right=QWidget(); layout=QVBoxLayout(right); self.selection=ui_text(QLabel, tr('화면의 물체를 선택하세요.')); self.selection.setWordWrap(True); layout.addWidget(self.selection)
        self.live_details=QPlainTextEdit(); self.live_details.setReadOnly(True); layout.addWidget(self.live_details)
        actions=QHBoxLayout(); self.recheck_button=button(tr('선택 물체 재검사'),lambda:self.guarded(self.recheck)); actions.addWidget(self.recheck_button)
        self.pick_button=button(tr('선택 물체 분류'),lambda:self.guarded(self.pick_selected)); actions.addWidget(self.pick_button); layout.addLayout(actions)
        layout.addWidget(button(tr('선택 물체 검사 원본 보기'),lambda:self.guarded(self.open_selected_record)))
        panes.addWidget(right); panes.setSizes([780,410]); self.counts=ui_text(QLabel, tr('검사한 물체 0  ·  재검사 0')); out.addWidget(self.counts)
    ...
    def make_equipment(self):
        ...
        out.addLayout(row); self.equipment_details=QPlainTextEdit(); self.equipment_details.setReadOnly(True); out.addWidget(self.equipment_details,1)
        row=FlowLayout()
