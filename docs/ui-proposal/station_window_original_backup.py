...
from PySide6.QtWidgets import QWidget,QVBoxLayout,QHBoxLayout,QLabel,QSplitter,QPlainTextEdit,QCheckBox,QDialog,QTableWidgetItem,QScrollArea,QFrame
...
class StationWindow(DesktopWindow):
    ...
    def make_live(self):
        content=QWidget(); out=QVBoxLayout(content); self.pages.addWidget(InspectionPage(content)); row=FlowLayout()
        self.start=button(tr('검사 시작'),lambda:self.guarded(self.begin)); self.pause=button(tr('검사 중단'),self.pause_inspection); self.start.setObjectName('inspectionStart')
        ...
        for w in (self.start,self.pause,self.unload,button(tr('카메라 영상'),self.open_preview),button(tr('현재 영상 수집'),lambda:self.guarded(self.capture_training)),button(tr('학습데이터 수집'),lambda:self.guarded(lambda:self.open_capture_studio(collect=True)))): row.addWidget(w)
        ...
        l.addWidget(ui_text(QLabel,tr('저장된 전체사진')))
        self.live_image=ImagePanel(); self.canvas=self.live_image.canvas; self.canvas.placeholder=tr('전체사진이 없습니다.\n검사 시작 후 촬영한 전체사진을 표시합니다.')
        self.canvas.selected.connect(self.select_track); l.addWidget(self.live_image,1)
        self.object_table=table([tr('물체 ID'),tr('판정'),tr('진행 상태')]); self.object_table.setMaximumHeight(165)
        self.object_table.itemSelectionChanged.connect(self.select_row); l.addWidget(self.object_table); panes.addWidget(left)
        right=QSplitter(Qt.Vertical)
        upper=QWidget(); u=QVBoxLayout(upper); u.setContentsMargins(0,0,0,0)
        self.selection=ui_text(QLabel,tr('화면의 물체를 선택하세요.')); u.addWidget(self.selection)
        self.detail_image=ImagePanel(); self.detail_image.canvas.placeholder=tr('선택한 물체의 상세사진이 여기에 표시됩니다.'); u.addWidget(self.detail_image,1); right.addWidget(upper)
        lower=QWidget(); d=QVBoxLayout(lower); d.setContentsMargins(0,0,0,0)
        d.addWidget(ui_text(QLabel,tr('기본 판정과 이유'))); self.live_details=QPlainTextEdit(); self.live_details.setReadOnly(True); d.addWidget(self.live_details,1)
        d.addWidget(ui_text(QLabel,tr('VLM 보조 설명'))); self.vlm_details=QPlainTextEdit(); self.vlm_details.setReadOnly(True); d.addWidget(self.vlm_details,1)
        d.addWidget(button(tr('VLM 설명 요청'),lambda:self.guarded(self.request_selected_vlm))); right.addWidget(lower)
        right.setSizes([380,320]); panes.addWidget(right); panes.setSizes([720,510]); self.counts=QLabel(); out.addWidget(self.counts)
    def make_products(self):
        super().make_products()
        self.pages.widget(1).widget().layout().insertWidget(1,button(tr('전체·상세 검사 설정'),lambda:self.guarded(self.edit_recipe)))
        self.pages.widget(1).widget().layout().insertWidget(2,button(tr('저장사진 검사'),lambda:self.guarded(self.open_photo_inspection)))
        self.pages.widget(1).widget().layout().insertWidget(3,button(tr('수동 도착 검사'),lambda:self.guarded(self.open_manual_inspection)))
    ...
    def select_track(self,identity):
        ...
        if key!=self.detail_key:
            self.detail_key=key; self.detail_image.canvas.pixmap=QPixmap(); self.detail_image.canvas.tracks=[]; self.detail_image.canvas.update()
            if target:
