...
class ImagePanel(QWidget):
    """Square image pane: optional title row, framed 1:1 canvas, zoom + overlay row.

    The canvas, its signals and every existing attribute name are unchanged; only
    the surrounding chrome was restyled for the MES shell. The frame is kept square
    because the workbench is square and the pipeline is fed a centre crop.
    """

    def __init__(self,canvas=None,title='',meta='',compact=False,square=True):
        ...
        self.title_label=QLabel(title); self.title_label.setObjectName('viewTitleStrong')
        self.meta_label=QLabel(meta); self.meta_label.setObjectName('viewMeta')
        head=QHBoxLayout(); head.setSpacing(6)
        head.addWidget(self.title_label); head.addStretch(1); head.addWidget(self.meta_label)
        if title or meta: layout.addLayout(head)
        else: self.title_label.hide(); self.meta_label.hide()

        self.frame=QWidget(); self.frame.setObjectName('viewFrame')
        self.frame.setAttribute(Qt.WA_StyledBackground,True)
        framed=QVBoxLayout(self.frame); framed.setContentsMargins(1,1,1,1); framed.setSpacing(0)
        framed.addWidget(self.canvas,1)
        if square:
            from .shell import SquareHolder
            self.holder=SquareHolder(self.frame); layout.addWidget(self.holder,1)
        else:
            self.holder=None; layout.addWidget(self.frame,1)

        controls=QHBoxLayout(); controls.setSpacing(4); controls.setContentsMargins(0,0,2,0)
        self.fit_button=button(tr('화면 맞춤'),self.canvas.fit)
        ...
        for widget in (self.fit_button,self.native_button,self.minus,self.plus):
            widget.setObjectName('zoomButton'); controls.addWidget(widget)
        self.scale_label=ui_text(QLabel); self.scale_label.setObjectName('zoomLevel')
        controls.addWidget(self.scale_label); controls.addStretch()
        # Narrow panes drop the checkbox caption; the tooltip carries the same text.
        self.overlays=QCheckBox() if compact else ui_text(QCheckBox, tr('검사 표시선'))
        ui_text(self.overlays.setToolTip, tr('검사 표시선'))
        self.overlays.setChecked(True)
        self.overlays.toggled.connect(self.toggle_overlays); controls.addWidget(self.overlays)
        layout.addLayout(controls)

        self.canvas.view_changed.connect(self.update_scale); self.update_scale()

    def set_title(self,title='',meta=''):
        self.title_label.setText(title); self.meta_label.setText(meta)
