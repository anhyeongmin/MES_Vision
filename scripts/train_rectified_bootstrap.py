...
def main():
 ...
 dataset=ROOT/'datasets/ash-real-rectified-bootstrap-v2'
 meta=json.loads((dataset/'provenance.json').read_text(encoding='utf-8'))
 assert meta['scope']=='train_only_unvalidated' and meta['images']==35
 ...
 assert len(ims)==35 and len(anns)==29
 ...
 out=ROOT/'artifacts/training/ash-real-rectified-v2/detail-defects';out.mkdir(parents=True,exist_ok=False)
 shutil.copyfile(dataset/'provenance.json',out/'label-snapshot.json')
 weights=ROOT/'artifacts/training/ash-real-bootstrap-v1/detail-defects/inference-unvalidated.pth'
 assert sha256(weights)=='b865c613dc43140c97deefef64c5325d6709150200ddaab34034561a887b1975'
 manifest={'status':'PREPARED','scope':'Initial fine-tune on visually inspected annotation drafts; independent evaluation pending','production_ready':False,'evaluation_performed':False,'validation_images':0,'train_images':len(ims),'annotations':len(anns),'epochs_requested':10,'initial_sha256':sha256(weights),'selection':'last epoch; no best-model metric','dataset':str(dataset),'preprocessing':'undistorted_full_frame_then_roi','calibration_sha256':meta['calibration_sha256'],'negative_images':6,'prior_eval_normal_reused':True}
