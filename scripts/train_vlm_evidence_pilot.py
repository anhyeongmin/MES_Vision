...
def prepare():return ROOT/'datasets/ash-vlm-evidence-pilot-v1/dataset.json'
...
from vlm_evidence_support import encode,self_test,SETTINGS
...
def main():
 ...
 out=ROOT/'artifacts/training'/('vlm-evidence-pilot-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'));out.mkdir(parents=True)
 manifest=dict(status='PREPARING',base=record,dataset=str(source),dataset_sha256=sha(source),train_examples=len(train),unique_train_images=len({x['image_sha256'] for x in train}),unique_real_train_images=8,eval_images=len(ev),epochs=args.epochs,production_ready=False,independent_evaluation=False,caption_status='assistant drafts',augmentation=SETTINGS,base_init='original2B not previous adapter',output=str(out),started_at=datetime.now().isoformat())
