...
SETTINGS={'quarter_turns':[0,90,180,270],'jitter_probability':.3,'jitter_degrees':10,'brightness_probability':.4,'brightness_range':[.9,1.1],'contrast_probability':.3,'contrast_range':[.9,1.1],'blur_probability':.15,'blur_radius':[.2,.5],'new_undistortion':False,'ground_truth_position_transformed':True,'reviewed_caption_jitter':0,'reviewed_caption_quarter_turns':True}
...
def rotate_caption(text,turns):
 # PIL positive quarter turns are counter-clockwise in image coordinates.
 mapping={'왼쪽':'아래쪽','아래쪽':'오른쪽','오른쪽':'위쪽','위쪽':'왼쪽'}
 for _ in range(turns):
  # Convert diagonal positions as units, then remaining wall directions.
  corners={'왼쪽 아래':'오른쪽 아래','오른쪽 아래':'오른쪽 위','오른쪽 위':'왼쪽 위','왼쪽 위':'왼쪽 아래'}
  protected={}
  for n,(a,b) in enumerate(corners.items()):
   token=f'@@C{n}@@';text=text.replace(a,token);protected[token]=b
  for n,a in enumerate(mapping):text=text.replace(a,f'@@D{n}@@')
  for n,b in enumerate(mapping.values()):text=text.replace(f'@@D{n}@@',b)
  for token,b in protected.items():text=text.replace(token,b)
 return text
...
def encode(processor,row,system,answer=True):
 ...
 if row.get('reviewed_caption'):
  turns=row.get('training_turn',0) if answer and row['split']=='train' else 0
  if answer and row['split']=='train':
   boxes=torch.tensor([box],dtype=torch.float32) if box is not None else torch.empty((0,4));im,boxes=geometry(im,boxes,turns,0)
  target=dict(row['target']);target['observation']=rotate_caption(target['observation'],turns)
 else:
  if answer and row['split']=='train':im,box=augment(im,box)
  target=caption(row['target']['code'],box,im.size)
...
def self_test():
 assert rotate_caption('오른쪽 위 원통과 오른쪽 벽',1)=='왼쪽 위 원통과 위쪽 벽'
 assert rotate_caption('왼쪽 아래와 오른쪽 위',4)=='왼쪽 아래와 오른쪽 위'
...
SETTINGS={"quarter_turns":[0,90,180,270],"deterministic_epoch_schedule":True,"jitter":False,"photometric":False,"caption_transformed":True}
