...
def main():
 ...
 dataset=ROOT/'datasets/ash-vlm-paired-ng04-v1/transfer.json';data=json.loads(dataset.read_text(encoding='utf-8'))
 rows=[r for r in data['items'] if r['target']['code'] in ['OK','NG04'] and r['image_sha256'] not in excluded];assert len(rows)==31
 data['scope']='Frozen paired-view adapter transfer diagnostic on 31 existing photos excluded by SHA256 from its training: same physical specimens, some same-video nearby frames, other prior-session frames; NOT independent field accuracy. No retraining. Code match is not grounded-description accuracy.'
 out=run/'transfer';out.mkdir(exist_ok=False);base=Path(meta['base']['path']);processor=AutoProcessor.from_pretrained(base,local_files_only=True);torch.set_num_threads(4);coord=GpuCoordinator(ROOT/'.cache/real-labeling/vlm/gpu-coordination');summaries=[];cards=[]
  ...
  for mode in ['after']:
   ...
   try:
    for n,row in enumerate([rows[0]]+rows):
     ...
     save(dest,dict(scope=data['scope'],model=mode,run=str(run),results=results));print(mode,n,'/31',round(seconds,2),raw,flush=True)
 ...
 page='<meta charset="utf-8"><style>body{font-family:sans-serif;max-width:1100px;margin:30px auto}article{border:1px solid #ccc;padding:10px;margin:10px}pre{white-space:pre-wrap}</style><h1>Qwen3.5-2B LoRA 전후 비교</h1><p>학습 장수와 설정은 run.json 참조. 8장 학습에서 제외된 기존 사진31장 점검입니다. 동일 영상의 인접 장면과 이전 촬영분을 포함하며, 독립적인 현장 평가가 아닙니다. 같은 실제 부품이므로 독립 현장 성능 검증은 아님. 정답 설명은 기존 영역과 시각 검토를 반영한 초안. 코드 일치와 설명 정확도는 별개. 예열 제외, 모델 상주, 생성 완료까지 측정. LoRA를 메모리에서 병합한 추론 결과이며 원본 모델은 수정하지 않음.</p><pre>'+html.escape(json.dumps(summaries,ensure_ascii=False,indent=2))+'</pre>'+''.join(cards)
