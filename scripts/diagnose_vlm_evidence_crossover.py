...
def main():
 ...
 dataset=ROOT/'artifacts/vlm-evidence-crossover-v1/inputs.json';data=json.loads(dataset.read_text(encoding='utf-8'));rows=data['items'];assert len(rows)==15
 out=ROOT/'artifacts/vlm-evidence-crossover-v1/results';out.mkdir(exist_ok=False);base=Path(meta['base']['path']);processor=AutoProcessor.from_pretrained(base,local_files_only=True);torch.set_num_threads(4);coord=GpuCoordinator(ROOT/'.cache/real-labeling/vlm/gpu-coordination');summaries=[];cards=[]
  ...
  for mode in ['after']:
   ...
   try:
    for n,row in enumerate([rows[0]]+rows):
     ...
     save(dest,dict(scope=data['scope'],model=mode,run=str(run),results=results));print(mode,n,'/15',round(seconds,2),raw,flush=True)
 ...
 page='<meta charset="utf-8"><style>body{font-family:Malgun Gothic,sans-serif;max-width:1100px;margin:30px auto}article{border:1px solid #ccc;padding:12px}pre{white-space:pre-wrap}</style><h1>사진·후보 교차 입력 점검</h1><p>후보와 영역 좌표 3조건 각각에 정상2장·NG04 2장·빈 영상 입력. 같은 조건 안에서는 사진만 변경. 이미 학습한 사진을 이용한 원인 분석이며 신규 성능 평가가 아님. 독립 현장 평가 아님. needs_review 빈도는 설명 정확도가 아니며 사진별 검토 필요. 정답 불량 코드는 입력하지 않음. 영상 없음 통제는 인공 사례로 실제 반사/애매함 검증을 대체하지 않음.</p><pre>'+html.escape(json.dumps(summaries,ensure_ascii=False,indent=2))+'</pre>'+''.join(cards)
