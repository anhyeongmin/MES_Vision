...
def main():
 ...
 parser.add_argument('--mode',choices=['real','mixed25'],required=True)
 ...
 from prepare_mixed_cad import prepare
 dataset=prepare()
 meta=json.loads((dataset/'provenance.json').read_text(encoding='utf-8'))
 assert meta['scope']=='train_only_unvalidated' and meta['real_images']==55 and meta['synthetic_images']>0
 ...
 assert len(ims)==meta['images'] and len(anns)>46
 ...
 manifest={'status':'PREPARED','scope':'Experimental mixed training: real annotations plus geometry-derived CAD boxes; independent evaluation pending','production_ready':False,'evaluation_performed':False,'validation_images':0,'train_images':55 if args.mode=='real' else len(ims),'available_dataset_images':len(ims),'annotations':46 if args.mode=='real' else len(anns),'epochs_requested':args.epochs,'initial_sha256':sha256(weights),'selection':'last epoch; no best-model metric','dataset':str(dataset),'preprocessing':'undistorted_full_frame_then_roi','calibration_sha256':meta['calibration_sha256'],'augmentation':__import__('rotation_training_support').SETTINGS,'negative_images':9 if args.mode=='real' else meta['negative_images'],'condition':args.mode,'sampling':('256 real' if args.mode=='real' else '192 real + 64 synthetic')+' per epoch','optimizer_steps_per_epoch':32,'real_images':55,'synthetic_images':0 if args.mode=='real' else meta['synthetic_images'],'prior_eval_normal_reused':True}
 ...
 try:
  ...
  with ExitStack() as stack:
   ...
   from ratio_ablation_support import install_ratio,self_test as mixing_test
   mixing_test();install_ratio(dm,meta,coco,args.mode)
   if args.check_only:
    dm.setup('fit')
   ...
   torch.save({'model':{k:v.detach().cpu() for k,v in module.model.state_dict().items()},'model_name':'RFDETRSmall','model_config':mc.model_dump(mode='json'),'args':{'num_classes':6,'class_names':codes},'mes_vision':{'role':'known_defect_detector','production_ready':False,'evaluation_performed':False,'training_mode':'mixed_real_cad_experimental_train_only','preprocessing':'undistorted_full_frame_then_roi','calibration_sha256':meta['calibration_sha256'],'synthetic':args.mode=='mixed25','contains_real_data':True}},dest)
