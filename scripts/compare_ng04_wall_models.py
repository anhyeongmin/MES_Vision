...
def report(out, all_results, jobs):
    ...
    page='''<!doctype html><meta charset="utf-8"><title>모델 비교 결과</title><style>body{font-family:sans-serif;max-width:1100px;margin:35px auto;padding:15px}td,th{padding:12px;border:1px solid #ccc}table{border-collapse:collapse}figure{display:inline-block;width:300px;vertical-align:top;margin:10px}img{max-width:100%;max-height:320px}figcaption{overflow-wrap:anywhere}</style><h1>NG04 벽 라벨 보완 학습 전후 비교</h1><p>before_wall: 기존 141장 학습 모델. after_wall: 같은 사진의 NG04 벽 라벨 16개를 수정하여 100에포크 추가 학습한 모델. 양쪽 모두 수정 라벨로 비교합니다. 추가 학습량도 달라 라벨 수정만의 효과로 해석할 수 없습니다.</p><p><strong>독립적인 성능 검증이 아닙니다.</strong> 두 모델이 학습에 사용한 동일한 141장과 그 변형으로 학습 상태와 취약점을 점검했습니다. 변형 이미지는 독립 표본이 아니며 실제 반사·원근 변화 전체를 재현하지 않습니다. 별도 촬영 없이 최적 모델이나 배포 가능 여부를 결정할 수 없습니다.</p><p>회전 4종 × 원본·어두움·밝음·흐림 = 모델마다 2256회. 종합 수치는 같은 사진의 반복 조건을 포함합니다. 임계값 비교는 진단용이며 프로그램 설정은 바꾸지 않습니다.</p><h2>원래 사진, 임계값 0.10</h2><table><tr><th>모델</th><th>정상 오탐 장수</th><th>불량 종류 미검출 장수</th><th>정답 박스 일치 IoU≥0.5</th></tr>'''+table+'''</table><p>박스 정답에는 초안과 동그라미 기반 근사 영역이 포함돼 위치 지표도 참고용입니다. 전체 조건·임계값 결과는 summary.csv, 모든 예측은 모델별 JSON에 저장됩니다.</p><h2>오류 예시: 모델당 최대 24장</h2><p>초록=정답 영역, 빨강=모델 예측. 정상 사진에는 초록 박스가 없습니다.</p>'''+''.join(pictures)
    (out/'report.html').write_text(page,encoding='utf-8')

def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--run',default='artifacts/training/ash-wall-reviewed-20260916-170042-480505/detail-defects')
    parser.add_argument('--baseline',default='artifacts/training/ash-recorded-20260914-212558-590266/detail-defects')
    parser.add_argument('--output',default='artifacts/wall-model-comparison-v1')
    ...
    with ExitStack() as stack:
        ...
        meta=json.loads((run/'run.json').read_text());assert meta['status']=='COMPLETED_UNVALIDATED' and meta['epoch']==100
        final=run/'inference-unvalidated.pth';assert sha(final)==meta['weights_sha256']
        from prepare_ng04_wall_review import prepare
        dataset=prepare();coco=json.loads((dataset/'train/_annotations.coco.json').read_text())
        provenance=json.loads((dataset/'provenance.json').read_text(encoding='utf-8'))
        jobs={}
        assert len(coco['images'])==len(provenance['items'])==141
        ...
        real_run=ROOT/args.baseline
        real_meta=json.loads((real_run/'run.json').read_text())
        real_weights=real_run/'inference-unvalidated.pth'
        assert sha(real_weights)==real_meta['weights_sha256']
        assert meta['initial_sha256']==sha(real_weights), 'New model must continue the baseline'
        assert real_meta['status']=='COMPLETED_UNVALIDATED'
        assert sha(final)=='f12a1615bb597cc1ba6c92c6f702d484895eb246ab542eb2ad81d3074643204f'
        sources={'before_wall':real_weights,'after_wall':final}
        ...
        if args.check_only:
            print('CHECK PASSED: two model hashes and 141 real images verified; no inference or training');return
        ...
        total=141*16*2;start=time.time();all_results={}
        print(f'OUTPUT: {out}\nModels: 2 | Images per model: 2256 | Total: {total}',flush=True)
        for model,source in sources.items():
            ...
            assert len(done)==len(results) and len(results)<=2256
            if len(results)==2256:
            ...
            try:
                ...
                for name,job in jobs.items():
                    ...
                    print(f'{model}: {len(results)}/2256 | elapsed {(time.time()-start)/60:.1f} min',flush=True)
            finally:
