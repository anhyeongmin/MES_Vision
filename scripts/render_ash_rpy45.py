...
def main():
    global OUT
    parser=argparse.ArgumentParser();parser.add_argument('--limit',type=int,default=0);parser.add_argument('--output',type=Path,default=ROOT/'datasets/ash-cad-rpy45-distance-v2')
    ...
    total=len(grid['roll'])*len(grid['pitch'])*len(grid['yaw'])*len(grid['distances_mm'])*4*7;assert total==2911104
    ...
    def conditions():
        for roll,pitch in pose_pairs:
          for distance_mm in grid['distances_mm']:
           for angle in grid['yaw']:
