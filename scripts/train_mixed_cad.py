...
def main():
 ...
 out=ROOT/'artifacts/training'/('ash-mixed-cad-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'))/'detail-defects';out.mkdir(parents=True,exist_ok=False)
 ...
 manifest={'status':'PREPARED','scope':'Experimental mixed training: real annotations plus geometry-derived CAD boxes; independent evaluation pending','production_ready':False,'evaluation_performed':False,'validation_images':0,'train_images':len(ims),'annotations':len(anns),'epochs_requested':args.epochs,'initial_sha256':sha256(weights),'selection':'last epoch; no best-model metric','dataset':str(dataset),'preprocessing':'undistorted_full_frame_then_roi','calibration_sha256':meta['calibration_sha256'],'augmentation':__import__('rotation_training_support').SETTINGS,'negative_images':meta['negative_images'],'sampling':'128 real + 128 synthetic per epoch; synthetic viewpoint weights 4/2/1','real_images':55,'synthetic_images':meta['synthetic_images'],'prior_eval_normal_reused':True}
 ...
 try:
  ...
  with ExitStack() as stack:
   ...
   from mixed_training_support import install_mixing,self_test as mixing_test
   mixing_test();install_mixing(dm,meta,coco)
   if args.check_only:
    dm.setup('fit')
   ...
   torch.save({'model':{k:v.detach().cpu() for k,v in module.model.state_dict().items()},'model_name':'RFDETRSmall','model_config':mc.model_dump(mode='json'),'args':{'num_classes':6,'class_names':codes},'mes_vision':{'role':'known_defect_detector','production_ready':False,'evaluation_performed':False,'training_mode':'mixed_real_cad_experimental_train_only','preprocessing':'undistorted_full_frame_then_roi','calibration_sha256':meta['calibration_sha256'],'synthetic':True,'contains_real_data':True}},dest)
