...
def encode(processor,row,system,answer=True):
 ...
 if row.get('reviewed_caption'):
  turns=random.randrange(4) if False else 0
  if False:im,box=augment(im,box,turns=turns,angle=0)
  target=dict(row['target']);target['observation']=rotate_caption(target['observation'],turns)
 else:
  if False:im,box=augment(im,box)
...
SETTINGS={"augmentation":False,"purpose":"exact four-image training fit"}
