...
def prepare():return ROOT/'datasets/ash-vlm-evidence-balanced-v2/dataset.json'
...
from vlm_evidence_balanced_support import encode,self_test,SETTINGS
...
def main():
 ...
 out=ROOT/'artifacts/training'/('vlm-evidence-balanced-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'));out.mkdir(parents=True)
 manifest=dict(status='PREPARING',base=record,dataset=str(source),dataset_sha256=sha(source),train_examples=len(train),unique_train_images=len({x['image_sha256'] for x in train}),unique_real_train_images=12,eval_images=len(ev),epochs=args.epochs,production_ready=False,independent_evaluation=False,caption_status='assistant drafts',augmentation=SETTINGS,base_init='original2B not previous adapter',output=str(out),started_at=datetime.now().isoformat())
 ...
 with FileLock(str(coord.root/'resident.lock'),timeout=0),coord.foreground(timeout=10):
  ...
  try:
   for epoch in range(1,args.epochs+1):
    ...
    if epoch in [4,8,args.epochs]:model.save_pretrained(out/f'adapter-epoch-{epoch:02}',safe_serialization=True)
