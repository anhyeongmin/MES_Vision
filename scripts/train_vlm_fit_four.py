...
def prepare():return ROOT/'datasets/ash-vlm-fit-four-v1/dataset.json'
...
from vlm_fit_four_support import encode,self_test,SETTINGS
...
def main():
 ...
 out=ROOT/'artifacts/training'/('vlm-fit-four-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'));out.mkdir(parents=True)
