...
from prepare_vlm_lora_aug import prepare,sha
...
from vlm_augmentation_support import encode,self_test,SETTINGS
...
def main():
 ...
 out=ROOT/'artifacts/training'/('vlm-2b-lora-aug-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'));out.mkdir(parents=True)
 manifest=dict(status='PREPARING',base=record,dataset=str(source),dataset_sha256=sha(source),train_images=86,eval_images=55,epochs=args.epochs,production_ready=False,independent_evaluation=False,caption_status='assistant drafts',augmentation=SETTINGS,base_init='original2B not previous adapter',output=str(out),started_at=datetime.now().isoformat())
