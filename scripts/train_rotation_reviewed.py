...
def main():
 ...
 out=ROOT/'artifacts/training'/('ash-rotation-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'))/'detail-defects';out.mkdir(parents=True,exist_ok=False)
 ...
 try:
  ...
  with ExitStack() as stack:
   ...
   if args.check_only:
    ...
    print('CHECK PASSED: all 55 augmented samples loaded; no training performed',flush=True);return
