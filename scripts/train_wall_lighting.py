...
def main():
 ...
 out=ROOT/'artifacts/training'/('ash-wall-lighting-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'))/'detail-defects';out.mkdir(parents=True,exist_ok=False)
 shutil.copyfile(dataset/'provenance.json',out/'label-snapshot.json')
 weights=ROOT/'artifacts/training/ash-wall-reviewed-20260916-170042-480505/detail-defects/inference-unvalidated.pth'
 assert sha256(weights)=='f12a1615bb597cc1ba6c92c6f702d484895eb246ab542eb2ad81d3074643204f'
 manifest={'status':'PREPARED','scope':'Experimental class-independent gamma and directional shading augmentation on existing 141 train crops; new evaluation clips excluded; no improvement claimed','production_ready':False,'evaluation_performed':False,'validation_images':0,'train_images':len(ims),'annotations':len(anns),'epochs_requested':args.epochs,'initial_sha256':sha256(weights),'selection':'last epoch; no best-model metric','dataset':str(dataset),'preprocessing':'undistorted_full_frame_then_roi','calibration_sha256':meta['calibration_sha256'],'augmentation':__import__('wall_lighting_training_support').SETTINGS,'negative_images':23,'sampling':meta['sampling'],'annotation_status':meta['annotation_status'],'prior_eval_normal_reused':True}
 ...
 try:
  ...
  coord=GpuCoordinator(ROOT/'artifacts/operation/vlm/gpu-coordination')
  with ExitStack() as stack:
   ...
   from wall_lighting_training_support import install,self_test
