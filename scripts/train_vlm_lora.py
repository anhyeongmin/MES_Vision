...
from prepare_vlm_lora import prepare,sha
...
def encode(processor,row,system,answer=True):
 with Image.open(row['image']) as im:im=im.convert('RGB');im.thumbnail((448,448));im=im.copy()
 ...
 full=processor.apply_chat_template(messages+[{'role':'assistant','content':json.dumps(row['target'],ensure_ascii=False,separators=(',',':'))}],tokenize=True,add_generation_prompt=False,return_dict=True,return_tensors='pt',enable_thinking=False)
 n=prefix['input_ids'].shape[1];assert torch.equal(full['input_ids'][:,:n],prefix['input_ids']),'Assistant prefix mismatch'
 labels=full['input_ids'].clone();labels[:,:n]=-100;assert (labels!=-100).sum()>5 and full['input_ids'].shape[1]<2048
 full['labels']=labels;return full
...
def main():
 ...
 source=prepare();data=json.loads(source.read_text(encoding='utf-8'));train=[r for r in data['items'] if r['split']=='train'];ev=[r for r in data['items'] if r['split']=='eval'];record=next(r for r in json.loads((ROOT/'models/vlm-benchmark-downloads.json').read_text()) if r['repo']=='Qwen/Qwen3.5-2B');base=Path(record['path']);assert record['revision']=='15852e8c16360a2fea060d615a32b45270f8a8fc'
 from datetime import datetime
 out=ROOT/'artifacts/training'/('vlm-2b-lora-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'));out.mkdir(parents=True)
 manifest=dict(status='PREPARING',base=record,dataset=str(source),dataset_sha256=sha(source),train_images=86,eval_images=55,epochs=args.epochs,production_ready=False,independent_evaluation=False,caption_status='assistant drafts',output=str(out),started_at=datetime.now().isoformat())
 ...
 print('PASS: 141 images and assistant-only target masks; no split overlap',flush=True)
