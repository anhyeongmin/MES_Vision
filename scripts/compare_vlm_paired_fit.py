...
def main():
 ...
 dataset=Path(meta['dataset']);assert sha(dataset)==meta['dataset_sha256'];data=json.loads(dataset.read_text(encoding='utf-8'));rows=[r for r in data['items'] if r['split']=='train'];assert len(rows)==8
  ...
  for mode in ['after']:
   ...
   try:
    for n,row in enumerate([rows[0]]+rows):
     ...
     save(dest,dict(scope=data['scope'],model=mode,run=str(run),results=results));print(mode,n,'/8',round(seconds,2),raw,flush=True)
