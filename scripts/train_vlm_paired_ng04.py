...
def prepare():return ROOT/'datasets/ash-vlm-paired-ng04-v1/dataset.json'
...
from vlm_paired_support import encode,self_test,SETTINGS

def main():
 parser=argparse.ArgumentParser();parser.add_argument('--epochs',type=int,default=5);parser.add_argument('--check-only',action='store_true');parser.add_argument('--smoke-test',action='store_true');args=parser.parse_args();assert 1<=args.epochs<=60
 ...
 out=ROOT/'artifacts/training'/('vlm-paired-ng04-'+datetime.now().strftime('%Y%m%d-%H%M%S-%f'));out.mkdir(parents=True)
  ...
  try:
   for epoch in range(1,args.epochs+1):
    ...
    for i,row in enumerate(order):
     row=dict(row,training_turn=(epoch-1)%4)
    ...
    if epoch in [5,15,args.epochs]:model.save_pretrained(out/f'adapter-epoch-{epoch:02}',safe_serialization=True)
